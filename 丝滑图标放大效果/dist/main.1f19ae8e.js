...
lis.forEach(function (li) {
  ...
  li.addEventListener("mouseover", function (e) {
    var item = e.target; // alert(item);

    var itemRect = item.getBoundingClientRect();
    var scale = Math.abs(itemRect.left - e.clientX) / itemRect.width;
    var prev = item.previousElementSibling || null;
    ...
    if (prev) {
      ...
      prev.style.setProperty("--scale", 1 + factor * Math.abs(scale - 1)); //   void prev.clientHeight;
    }
    ...
    if (next) {
      ...
      next.style.setProperty("--scale", 1 + factor * scale); //   void next.clientHeight;
    }
  });
}); //鼠标不在导航栏时，移开

document.querySelector(".dock").addEventListener("mouseleave", function (e) {
  resetScale();
});
